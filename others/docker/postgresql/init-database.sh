...
# This script enables pgvector and sets up privileges.
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "${POSTGRES_DB:-interview_me}" <<-EOSQL
    CREATE EXTENSION IF NOT EXISTS vector;

    GRANT ALL PRIVILEGES ON DATABASE ${POSTGRES_DB:-interview_me} TO $POSTGRES_USER;
